...
**注意：yolov5中backbone的C3模块检测较慢，且yolov5m参数较大，算法测试fps会相对很低，所以总的算法测试分数可能还没有yolov5s高；对检测速度要求高的部署可直接选取轻量化模型(fasterrcnn,fastestnet)，或者对yolo主干部分轻量化处理(shuffle,mobile,深度可分离等)(如果tensorrt版本不支持一些结构处理，可能要分离backbone做单独处理)，tensorrt可选取(INT_8,效果一般)**

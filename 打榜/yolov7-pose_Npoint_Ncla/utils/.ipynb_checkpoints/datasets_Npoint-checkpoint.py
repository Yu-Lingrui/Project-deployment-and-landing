    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_images:
            ...
            for i, x in pbar:
                ...
                self.imgs[i] = None if self.imgs[i] is None else np.array(cv2.imencode('.jpg', self.imgs[i])[1])
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = None if img is None else cv2.imdecode(im, cv2.IMREAD_COLOR)
        return img, self.im_hw0[index], self.im_hw[index]  # img, hw_original, hw_resized

...
def model_inference_image(model_path):
    ...
    result_json = post_process(output[0], score_threshold=0.05)
...
def post_process(output, score_threshold=0.3):
    ...
    return json.dumps(
        {'algorithm_data': {'is_alert': is_alert, 'target_count': target_count, 'target_info': target_info},
         'model_data': {"objects": objects}}, indent=1)
...
if __name__ == '__main__':
    ...
    result = model_inference_image(model_path=args.model_path)
    f2 = open('test.json', 'w', newline='\n')
    f2.write(result)
    f2.close()
    #log.info(result)

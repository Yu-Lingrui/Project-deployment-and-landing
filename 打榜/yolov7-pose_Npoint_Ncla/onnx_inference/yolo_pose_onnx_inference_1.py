...
def model_inference_image_list(model_path, img_path=None, mean=None, scale=None, dst_path=None):
    ...
    for img_index, img_file  in pbar:
        ...
        post_process(img_file, dst_file, output[0], score_threshold=0.05)


def post_process(img_file, dst_file, output, score_threshold=0.1):
    """
    ...
    for idx in range(len(det_bboxes)):
        ...
        kpt = kpts[idx][5:]
        #############################
        # det_scores[0:1] = 1. / (1. + np.exp(det_scores[0:1]))
        # det_labels[0:1] = 1. / (1. + np.exp(det_labels[0:1]))
        # det_bbox[0:1,0:1] = 1. / (1. + np.exp(det_bbox[0:1,0:1]))
        # kpt[0:1,0:1] = 1. / (1. + np.exp(kpt[0:1,0:1]))
        # det_score = max(det_scores[idx])
        # det_label = max(det_labels[idx])
        # det_bbox = det_bbox[np.arange(4), np.argmax(det_bbox, axis=1)]  # 取每一行的最大值cla
        # kpt = kpt[np.arange(58), np.argmax(kpt, axis=1)]  # 取每一行的最大值cla
        #normalized = np.array(coords, dtype=np.float32)
        kpt[0::3] *= (img.shape[1] / 640)
        kpt[1::3] *= (img.shape[0] / 640)
        det_bbox[0::2] *= (img.shape[1] / 640)
        det_bbox[1::2] *= (img.shape[0] / 640)
        #############################
        if det_scores[idx] > 0:

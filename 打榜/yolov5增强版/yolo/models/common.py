...
class SKAttention(nn.Module):
    ...
    def __init__(self, c1,c2,kernels=[1, 3, 5, 7],reduction=16,group=1,L=32):
        super().__init__()
        self.d=max(L,c2//reduction)
        self.convs=nn.ModuleList([])
        for k in kernels:
            self.convs.append(
                nn.Sequential(OrderedDict([
                    ('conv',nn.Conv2d(c2,c2,kernel_size=k,padding=k//2,groups=group)),
                    ('bn',nn.BatchNorm2d(c2)),
                    ('relu',nn.ReLU())
                ]))
            )
        self.fc=nn.Linear(c2,self.d)
        self.fcs=nn.ModuleList([])
        for i in range(len(kernels)):
            self.fcs.append(nn.Linear(self.d,c2))

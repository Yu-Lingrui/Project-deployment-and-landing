...
JiErrorCode SampleAlgorithm::Process(const cv::Mat &in_frame, const char *args, JiEvent &event)
{
    ...
    for (auto &obj : detected_objects)
    {
        if (obj.label == 2 || obj.label == 4)
        {
            cv::Rect box = cv::Rect(obj.x1, obj.y1, obj.x2 - obj.x1, obj.y2 -obj.y1);
            for (auto &det : detected_objects)
            {
                if ((det.label == 0 || det.label == 1)  && ((box & cv::Rect(det.x1, det.y1, det.x2 - det.x1, det.y2 -det.y1)).area() == box.area()))
                {
                    valid_targets.emplace_back(obj);
                    break;
                }

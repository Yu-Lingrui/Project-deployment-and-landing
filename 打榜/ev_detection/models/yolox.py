...
class DetectX(nn.Module):
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        xy, wh, etc = outputs.split((2, 2, self.num_classes + 1), 2)  # tensor_split((2, 4, 5), 4) if torch 1.8.0
        xy = (xy * 2 + grids) * strides  # xy
        wh = torch.exp(wh)*strides # wh
        out = torch.cat((xy, wh, etc), 2)
